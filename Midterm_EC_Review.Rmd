...
sum(is.na(school$PerctBlended))

#Answer the question as a comment:
  ##Number of NA values in PerctBlended: 3


#Code: Change NA values to 0 only in PerctBlended

school$PerctBlended[is.na(school$PerctBlended)] = 0
...
school$District = as.factor(school$District)
school$Disaggregation.Category = as.factor(school$Disaggregation.Category)
school$Category = as.factor(school$Category)
...
schoolAll = subset(school, school$Category == "All District")
...
#Answer the following as a comment:
  ##Average Total Enrollment:  25051 students       
  ##Average PerctBlended: 0.2950  or 29.5%           
  ##Average PerctRemote:   0.7050 or 70.5%           
  ##Based on the results of PerctBlended and PerctRemote, what is the preference-- Blended Learning or Remote Learning?: Remote
...
#Code: next line using schoolAll
tapply(schoolAll$NumRemote,schoolAll$District,mean)

#Answer the following as a comment:
  ##District Number with Highest NumRemote: District 2 
...
cor(tesla$Close,tesla$Open)
cor(tesla$Close,tesla$High)
cor(tesla$Close,tesla$Low)
cor(tesla$Close,tesla$Volume)

#Answer the following as a comment:
  ##Highest correlation co-efficient (comment the column name): Low
  ##Second highest correlation co-efficient (comment the column name): Open
...
#Answer the following as a comment (no code this chunk):
  #Answer (a. Regression Equation): Y = 59.27306 + 0.91617x
  #Answer (b. R-Squared): 0.892 or 89.2 percent is explained
  #Answer (c. Hypothesis Testing): answer the p-value and Decision
    #p-value (< or > alpha of 0.05): p value = 0.0000000000000002 < 0.05 
    #Decision: Reject the null hypothesis [that Low is not significant]; Low is significant in predicting Close values
...
predict(R1, data.frame(Low = 95), interval="confidence")

#Answer the following as a comment:
  ##Answer (Response): With 95% confidence, we can say tesla stock with a 95 low would have a close value between 137.6889 and 154.9295
